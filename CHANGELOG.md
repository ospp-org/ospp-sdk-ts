...
`errorText` carried three limits (64 ×1, 128 ×17, 256 ×3) for one meaning that is actually
...
characters, none over 32 — 2.3× headroom.

**Corpus: 291/291, unchanged.** Measured before and after, with the floor pinned in both
directions (at 26 exactly one vector fails; at 8, eighteen do), so the green is a measurement
rather than a keyword the corpus never exercises.

No library code moves. `OSPP_PROTOCOL_VERSION` stays `0.3.0`.
...
**Two claims about the pinned spec were false in the published `0.37.0`, and the gate that catches
them was run before the change that broke them rather than after.** `README.md` said `v0.37.3` and
`SessionEndReason.ts` said `0.37.3` while `.spec-ref` read `v0.38.0`; `check:doc-claims` reports
exactly this and was green when it ran, because `.spec-ref` moved afterwards in the same session.
Both sentences now derive from `.spec-ref`, and the gate reads **14 of 14 claims agree**. The
lesson is the ordering, not the strings: a claim-derivation gate has to be the LAST thing run
before a tag, not the last thing run before the final edit.
